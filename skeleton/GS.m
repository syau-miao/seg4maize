...
%         MIN_NEIGHBOR_NUM =8;%8
%         MAX_NEIGHBOR_NUM =30;% orginal 30
         MIN_NEIGHBOR_NUM =6;%8
         MAX_NEIGHBOR_NUM =6;% orginal 30
        LAPLACIAN_CONSTRAINT_WEIGHT = 30; % init Laplacian contraction weight, compute automatically now.
        POSITION_CONSTRAINT_WEIGHT = 30; % init position constraint weight
        LAPLACIAN_CONSTRAINT_SCALE = 10; % scalar for increasing WL in each iteration
        ...
        CONTRACT_TERMINATION_CONDITION = 0.001;% contract Termination Conditions for total area ratio 0.01         
    end
...
%                     wl = max(3, 0.1*(npts)^0.5); % original guess of JJCAO                        
                ...
                otherwise % combinatorial or mvc
                    if GS.USING_POINT_RING
                        ms = GS.one_ring_size(PS.pts,PS.rings,3);
                        wl = 1.0/(5.0*mean(ms));
%                     wl = max(3, 0.1*(npts)^0.5); % original guess of JJCAO                        
                    else                        
                        wl = 1.0/(10.0*sqrt(GS.average_face_area(PS.pts,PS.faces))); % implementation of Oscar08    
